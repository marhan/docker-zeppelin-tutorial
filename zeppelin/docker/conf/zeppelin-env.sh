...
export SPARK_SUBMIT_OPTIONS="--jar $(JARS=("/opt/libs"/*.jar); IFS=,; echo "${JARS[*]}") --driver-class-path $(JARS=("/opt/libs"/*.jar); IFS=,; echo "${JARS[*]}") --properties-file /opt/properties/spark.properties"
echo "############################################################"
echo
